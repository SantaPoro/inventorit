...
export default class LoginGoogleMutationResolver {
	...
	async loginGoogle(@Ctx() context: Context, @Arg('input') { code }: LoginGoogleInput): Promise<LoginGooglePayload> {
		if (context.user) {
		...
		const profile = JSON.parse(Buffer.from(result.data['id_token'].split('.')[1], 'base64').toString('utf8'));
